...
/**
 * An abstract computerized com.example.degrood21.mexican_train_dominoes.game player player. This is an abstract class, that
...
public abstract class GameComputerPlayer implements GamePlayer, Tickable {
	/**
	 * the current com.example.degrood21.mexican_train_dominoes.game state
	 */
	protected Game game; // the com.example.degrood21.mexican_train_dominoes.game object
	...
	private boolean gameOver = false; // whether the com.example.degrood21.mexican_train_dominoes.game is over
	private GameMainActivity myActivity; // the com.example.degrood21.mexican_train_dominoes.game's main activity, set only
			// this com.example.degrood21.mexican_train_dominoes.game is connected to the GUI
	private GameTimer myTimer = new GameTimer(this); // my timer
	
	/**
	 * Returns this com.example.degrood21.mexican_train_dominoes.game's timer.
	 * 
	 * @return this com.example.degrood21.mexican_train_dominoes.game's timer object
	 */
	 ...
	 * 
	 * @param info
	 * 			the object representing the information from the com.example.degrood21.mexican_train_dominoes.game
	 */
	...
	private class MyRunnable implements Runnable {
		...
		// the com.example.degrood21.mexican_train_dominoes.game, or the timer
		public void run() {
			
			// if com.example.degrood21.mexican_train_dominoes.game is over, do nothing
			if (gameOver) return;
			...
			if (data instanceof GameInfo) { // ignore non GameInfo objects
				...
				if (game == null) {
					...
					// CASE 1: we don't know who our com.example.degrood21.mexican_train_dominoes.game is; the only thing we're
					...
					if (myInfo instanceof BindGameInfo) {
						...
						game = bgs.getGame(); // set our com.example.degrood21.mexican_train_dominoes.game
						playerNum = bgs.getPlayerNum(); // set our player ID
						
						// send a message to the com.example.degrood21.mexican_train_dominoes.game with our player's name
					...
					if (myInfo instanceof StartGameInfo) {
						...
						initAfterReady();
						// tell com.example.degrood21.mexican_train_dominoes.game that we're ready to play
						game.sendAction(new ReadyAction(GameComputerPlayer.this));
					}
				}
				else if (myInfo instanceof GameOverInfo) {
					
					// CASE 3: we get a "com.example.degrood21.mexican_train_dominoes.game over" message
					
					// if we are the GUI, pop up a message box and tell the
					// activity that the com.example.degrood21.mexican_train_dominoes.game is over
					if (myActivity != null) {
						gameIsOver(((GameOverInfo)myInfo).getMessage());
						myActivity.setGameOver(true);
					}
					
					// acknowledge to the com.example.degrood21.mexican_train_dominoes.game that we have receive the message
					game.sendAction(new GameOverAckAction(GameComputerPlayer.this));
					
					// mark com.example.degrood21.mexican_train_dominoes.game as being over
					gameOver = true;
				}
