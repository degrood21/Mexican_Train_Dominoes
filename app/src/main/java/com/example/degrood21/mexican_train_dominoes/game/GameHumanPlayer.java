...
import com.example.degrood21.mexican_train_dominoes.game.infoMsg.TimerInfo;
import com.example.degrood21.mexican_train_dominoes.game.util.GameTimer;
import com.example.degrood21.mexican_train_dominoes.game.util.MessageBox;
 ...
 * particular com.example.degrood21.mexican_train_dominoes.game, a subclass should be created that can display the current
 * com.example.degrood21.mexican_train_dominoes.game state and responds to user commands.
 * 
...
public abstract class GameHumanPlayer implements GamePlayer, Tickable {
	 ...
	 * instance variables
	 */
	protected Game game; // the com.example.degrood21.mexican_train_dominoes.game
	...
	private boolean gameOver; // whether the com.example.degrood21.mexican_train_dominoes.game is over
	...
	public GameHumanPlayer(String name) {
		...
		// mark com.example.degrood21.mexican_train_dominoes.game as not being over
		this.gameOver = false;
	...
	/**
	 * Returns this object's com.example.degrood21.mexican_train_dominoes.game timer
	 * 
	 * @return this object's com.example.degrood21.mexican_train_dominoes.game timer.
	 */
	 ...
	 * knows what their com.example.degrood21.mexican_train_dominoes.game-position and opponents' names are.
	 */
	 ...
	 * Abstract Methods
	 * 
	 * Create the com.example.degrood21.mexican_train_dominoes.game specific functionality for this human player by
	...
	/**
	 * Sends a 'state' object to the com.example.degrood21.mexican_train_dominoes.game's thread.
	 * 
	 * @param info
	...
	private class MyRunnable implements Runnable {
		...
		public void run() {
			...
			// if the com.example.degrood21.mexican_train_dominoes.game is over, just tell the activity that the com.example.degrood21.mexican_train_dominoes.game is over
			if (gameOver) {
			...
			if (game == null) {
				// com.example.degrood21.mexican_train_dominoes.game has not been bound: the only thing we're looking for is
				// BindGameInfo object; ignore everything else
				if (myInfo instanceof BindGameInfo) {
					Log.i("GameHumanPlayer", "binding com.example.degrood21.mexican_train_dominoes.game");
					BindGameInfo bgs = (BindGameInfo)myInfo;
					game = bgs.getGame(); // set the com.example.degrood21.mexican_train_dominoes.game
					playerNum = bgs.getPlayerNum(); // set our player id
					
					// respond to the com.example.degrood21.mexican_train_dominoes.game, telling it our name
				...
				if (myInfo instanceof StartGameInfo) {
					Log.i("GameHumanPlayer", "notification to start com.example.degrood21.mexican_train_dominoes.game");
					
					// update our player-name array
					allPlayerNames = ((StartGameInfo)myInfo).getPlayerNames();

					// perform com.example.degrood21.mexican_train_dominoes.game-specific initialization
					initAfterReady();
					
					// tell the com.example.degrood21.mexican_train_dominoes.game we're ready to play the com.example.degrood21.mexican_train_dominoes.game
					game.sendAction(new ReadyAction(GameHumanPlayer.this));
				}
			}
			else if (myInfo instanceof GameOverInfo) {
				// if we're being notified the com.example.degrood21.mexican_train_dominoes.game is over, finish up
				...
				// acknowledge to the com.example.degrood21.mexican_train_dominoes.game that the com.example.degrood21.mexican_train_dominoes.game is over
				game.sendAction(new GameOverAckAction(GameHumanPlayer.this));
				
				// set our instance variable, to indicate the com.example.degrood21.mexican_train_dominoes.game as over
				gameOver = true;
			}
	...
	/**
	 * callback method--called when we are notified that the com.example.degrood21.mexican_train_dominoes.game is over
	 * 
	 * @param msg
	 * 		the "com.example.degrood21.mexican_train_dominoes.game over" message sent by the com.example.degrood21.mexican_train_dominoes.game
	 */
	protected void gameIsOver(String msg) {
		// the default behavior is to put a pop-up for the user to see that tells
		// the com.example.degrood21.mexican_train_dominoes.game's result
