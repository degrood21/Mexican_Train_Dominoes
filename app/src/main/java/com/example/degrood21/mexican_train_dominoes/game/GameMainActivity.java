...
import com.example.degrood21.mexican_train_dominoes.R;
...
import com.example.degrood21.mexican_train_dominoes.game.util.IPCoder;
...
View.OnClickListener {
	...
	// A reference to the object representing the com.example.degrood21.mexican_train_dominoes.game itself. This is the
	// object that knows the rules of the com.example.degrood21.mexican_train_dominoes.game. This variable is initialized in
	// launchGame.
	private Game game = null;

	// an array containing references to all the players that are playing the com.example.degrood21.mexican_train_dominoes.game
	...
	// whether the com.example.degrood21.mexican_train_dominoes.game is in the "configuration" stage, before the actual com.example.degrood21.mexican_train_dominoes.game
	// has started
	private boolean doingConfiguration = true;

	/**
	 * contains the com.example.degrood21.mexican_train_dominoes.game configuration this activity will be used to initialize
	 */
...
//		}
	...
	private final String launchGame(GameConfig config) {
		...
		// Set the title text with the com.example.degrood21.mexican_train_dominoes.game's name
		this.setTitle(config.getGameName());

		// create the com.example.degrood21.mexican_train_dominoes.game if it's local (we defer remote com.example.degrood21.mexican_train_dominoes.game creation
		...
		// create the com.example.degrood21.mexican_train_dominoes.game if it's remote
		if (!config.isLocal()) { // remote com.example.degrood21.mexican_train_dominoes.game
			game = createRemoteGame(config.getIpCode());
			// verify we have a com.example.degrood21.mexican_train_dominoes.game
			if (game == null) {
				return "Could not find com.example.degrood21.mexican_train_dominoes.game server on network.";
			}
		}
		...
		// start the com.example.degrood21.mexican_train_dominoes.game; then return null to indicate that the launch was
		// successful
		game.start(players);
		return null;
	...
	protected void initStarterGui() {
		...
		// Set the title text using the com.example.degrood21.mexican_train_dominoes.game's name
	...
	public void onClick(View button) {
		...
		//Save Config Button
			...
			if (configTemp.saveConfig(saveFileName(), this)) {
				...
				MessageBox.popUpMessage("Unable to save com.example.degrood21.mexican_train_dominoes.game configuration.", this);
			}
		}
	 ...
	 * com.example.degrood21.mexican_train_dominoes.game is already over).
	 */
	@Override
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		if (keyCode == KeyEvent.KEYCODE_BACK && !gameIsOver) {
			// We have seen the back-key pressed, and the com.example.degrood21.mexican_train_dominoes.game is not over;
			...
			// otherwise (not BACK key, or com.example.degrood21.mexican_train_dominoes.game is over), allow superclass method
			// to handle it
